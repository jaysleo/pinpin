<template>
	<view class="qglist">
		<view class="item" v-for="(item,index) in list" :key="index" v-if="item.product.length">
			<view class="hd">
				<view class="u-flex-1">
			...
			<view class="bd">
				<view class="shopinfo">
					<u-image :src="item.pic" width="90" height="90"></u-image>
					<view class="shopinfo_name">
						<view class="txt">{{item.name}}</view>
						<view class="tag">
							<text class="itag orange">低至{{item.min_discount}}折</text>
							<text class="itag orange">{{item.product.length}}个单品</text>
						</view>
					</view>
				</view>
				<view class="prolist">
